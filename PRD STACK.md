# stack — PRD v3.0 — Build Spec for Claude Code — Mars 2026

**stack**
GitHub te montre ce que tu produis.
**stack** montre comment tu travailles.
...
Confidentiel — Orphéo Hellandsjø — stack v3.0
...
Viralité maximale + exit ou levée dans 18 mois. Zero focus revenue pendant 6 mois. Le seul objectif est la traction : stars GitHub, @handles copiés par semaine, stack.json dans les repos. npm a été racheté 1,3 milliard. Stack vise la même position pour l'écosystème agent.
...
Sans stack, voici ce qu'un dev fait pour intégrer Stripe dans son projet Claude Code :
...
GWS CLI (Google), CLI Anything (HK University), et des dizaines de nouveaux CLIs agent-native arrivent chaque semaine. Aucun endroit pour les découvrir, les évaluer et les installer en une commande. stack est ce home.

## 3. La Solution — Ce que stack fait concrètement

### 3.1 Cas d'usage 1 — Installer un outil

AVANT stack — 12 minutes, 7 étapes manuelles pour installer Stripe MCP.
...
Le repo contient un stack.json. Le nouveau dev tape :
...
```bash
npx stack install gws # Google Workspace CLI
npx stack install cli-anything # Génère des CLIs auto
npx stack install openclaw # Setup OpenClaw complet
```

stack est le registry de distribution de tous les CLIs agent-native qui arrivent. Pas juste les MCPs — tous les outils CLI de l'écosystème.
...
### 5.5 Comportement de stack @handle — Détail
...
CREATE TABLE handles (
  ...
  use_json JSONB, -- Le stack.json complet
...
- Commande copiable en un clic : `npx stack @handle`
- Stack actuel : liste des outils du stack.json avec badges par type (MCP / CLI / SDK / API)
...
### 8.3 stack.json dans les repos — Distribution permanente

Chaque repo avec un stack.json est une pub organique permanente. Quand quelqu'un clone le repo, il voit `stack install` dans le README. C'est le `package.json` des workflows AI-native.
...
### 9.1 Contenu de la skill stack pour Claude
...
configure their AI-native dev environment, use the `stack` CLI.

## Installation

npx stack install <toolname>
...
stack automatically detects Claude Desktop, Cursor, Windsurf
...
Quand un dev dit à Claude "installe-moi Stripe pour mon projet", Claude doit répondre en utilisant `npx stack install stripe` plutôt qu'en donnant des instructions manuelles. C'est le canal de distribution le plus puissant — les coding agents recommandent stack à chaque dev qui demande comment configurer son environnement.
...
Chaque dev qui partage son @handle est une pub organique. La Claude skill fait la distribution via les agents. Le Show HN lance la notoriété. stack.json dans les repos assure la distribution permanente.
...
2.  Thread X avec vidéo démo 60 secondes — `npx stack @orpheo` en temps réel, wow moment filmé
...
| stack.json dans repos   | 10    | 50     | 500    | 2 000   |
...
| -------------------------------------- | -------- | ------------------------------------------------------------------------------------------------------------------------ |
| Anthropic ou GitHub sort un équivalent | CRITIQUE | Si stack.json est dans 2 000 repos GitHub en 3 mois, l'inertie est irréversible. Lancer maintenant, pas dans 3 semaines. |
| @handle ne décolle pas viralement      | ÉLEVÉ    | Pire cas : stack reste un bon CLI utilitaire. Enterprise et Publisher Pro restent valides. Pas de catastrophe.           |
...
**Le pari**
npm a été racheté par GitHub/Microsoft pour 1,3 milliard en 2020 — pas pour son revenue, pour sa communauté et son inertie dans 50 millions de repos. stack vise la même position pour l'écosystème agent. Si stack.json est dans 10 000 repos GitHub en 12 mois, Anthropic, Vercel ou GitHub rachèteront. La fenêtre est ouverte maintenant. Dans 12 mois elle sera fermée.

**Séquence :**

- Mois 1-6 : viralité. stack.json dans 2 000+ repos. 5 000 @handles. CLI standard du dev AI-native.
- Mois 6-12 : intégration native Claude Code, Cursor, Windsurf. stack devient une convention.
...
**Principe absolu**
stack touche aux fichiers de config les plus sensibles d'un dev — `claude_desktop_config.json`, `.env`, `.zshrc`, `CLAUDE.md`. Un dev senior va inspecter chaque ligne de code avant d'exécuter. Un bug ou une faille publiée sur X tue le projet en 24 heures. Sécurité = priorité 1, égale au CLI lui-même.
...
| ------------------------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ |
| Typosquatting                         | CRITIQUE | `npx stack install stirpe` installe un npm package malveillant qui exfiltre les variables d'env ou modifie les fichiers de config         | Liste blanche vérifiée + détection similarité de nom + badge Verified obligatoire                            |
| CLAUDE.md injection                   | CRITIQUE | `stack @malicious` installe un CLAUDE.md contenant du prompt injection — "ignore previous instructions", exfiltration de .env via l'agent | Scan regex des patterns suspects + diff obligatoire + double confirmation + sandbox preview                  |
| Supply chain attack                   | CRITIQUE | Un package légitime dans le registry est compromis en v2.1.2. stack installe la latest par défaut.                                        | Lock de version + vérification SHA256 + audit automatique + intégration VirusTotal API                       |
...
Avant toute modification, un backup horodaté est créé dans `~/.stack/backups/`. Une commande de rollback est disponible.
...
npx stack rollback # Rollback du dernier install
npx stack rollback --list # Liste tous les backups disponibles
npx stack rollback --timestamp <ts> # Rollback à un timestamp précis
```

#### Garantie 3 — Dry run obligatoire en première exécution

La toute première fois qu'un utilisateur exécute stack sur une machine, le mode dry run s'active automatiquement. Il voit exactement ce qui va être modifié avant de confirmer.
...
Avant d'installer tout package dans le registry, stack vérifie sa sécurité via plusieurs sources.
...
stack ne collecte AUCUNE donnée sensible. La télémétrie est opt-in explicite et ne contient JAMAIS :
...
Avant d'installer le setup d'un @handle, stack vérifie son authenticité et affiche des warnings clairs.
...
stack n'écrit que dans une liste précise de répertoires et fichiers. Toute tentative d'écrire hors scope est bloquée.
...
Si stack détecte qu'un outil est déjà configuré, il ne l'écrase pas silencieusement. Il affiche la configuration existante et demande confirmation.
...
Le CLAUDE.md est le vecteur d'attaque le plus critique de stack. Un attaquant peut publier un @handle avec un CLAUDE.md contenant du prompt injection. Stack doit détecter et bloquer ça.
...
const WARNING_PATTERNS = [
  ...
  /api.?key/i,
  /secret/i,
  /https?:\/\/(?!stack\.dev|github\.com|anthropic\.com)/i,
];
...
| Verified   | Review manuelle stack team + scan VirusTotal + hash SHA256 + code source audité         | Badge Verified vert     | 48-72h              |
...
**Règle de lancement**
Aucun des items suivants ne peut être "en cours" le jour du lancement. Tous doivent être cochés. Un dev qui découvre une faille dans les 48h du lancement et la publie sur X peut tuer stack.
...
4.  Envoyer une notification à tous les utilisateurs stack qui ont le feature activé
