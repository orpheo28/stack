    ...
    <main className="min-h-screen">
      ...
      <div className="mx-auto max-w-3xl px-6 py-20">
        ...
        <div className="mb-12">
          ...
          <h1 className="text-3xl font-bold tracking-tight text-zinc-100 mt-4">Trending tools</h1>
          <p className="text-sm text-zinc-500 mt-2">Most installed AI-native tools this week</p>
        </div>
        ...
        {items.length === 0 ? (
          <div className="rounded-lg border border-dashed border-zinc-800 px-6 py-10 text-center">
            <p className="text-sm text-zinc-600">No tools found for this filter.</p>
          </div>
        ) : (
          <div className="space-y-1">
            {items.map((t, i) => (
              <Link
                key={t.name}
                href={`/install/${t.name}`}
                className="group flex items-center gap-4 rounded-lg px-4 py-3.5 -mx-4 transition-colors hover:bg-zinc-900/70"
              >
                <span className="text-xs text-zinc-600 w-6 text-right font-mono tabular-nums">
                  {i + 1}
                </span>
                <div
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {t.description !== null && (
                    <p className="text-xs text-zinc-600 truncate mt-0.5">{t.description}</p>
                  )}
                </div>
        ...
        <div className="mt-12 pt-6 border-t border-zinc-900 text-center">
          <p className="text-xs text-zinc-600">
            Install any tool:{' '}
            <code className="text-zinc-500 font-mono">npx usedev install &lt;name&gt;</code>
          </p>
        </div>
      </div>
    </main>
